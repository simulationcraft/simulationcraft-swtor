...
{
  int size = to_seconds<int>( sim -> max_time * ( 1 + sim -> vary_combat_length ) );
                          ...
                          result_type result )
{
  ...
  timeline_amount[ to_seconds<int>( sim -> current_time ) ] += act_amount;
}
...
void stats_t::combat_end()
{
  ...
  portion_aps.add( player -> iteration_fight_length != timespan_t::zero() ?
        iteration_actual_amount / to_seconds( player -> iteration_fight_length ) : 0 );
...
void stats_t::analyze()
{
  ...
  frequency = num_intervals ? to_seconds( total_intervals ) / num_intervals : 0;
  ...
  if ( compound_amount > 0 )
  {
    ...
    aps  = ( total_time > timespan_t::zero() ) ? ( compound_amount / to_seconds( total_time ) ) : 0;

    total_time = total_execute_time + ( channeled ? total_tick_time : timespan_t::zero() );
    apet = ( total_time > timespan_t::zero() ) ? ( compound_amount / to_seconds( total_time ) ) : 0;
  ...
  ttpt = num_ticks ? to_seconds( total_tick_time ) / num_ticks : 0;
  etpe = num_executes? ( to_seconds( total_execute_time ) + ( channeled ? to_seconds( total_tick_time ) : 0 ) ) / num_executes : 0;
